...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validated = $request->validate([
                ...
                'correo_institucional'   => ['required', 'string', 'email', 'max:100', 'unique:usuarios,correo_institucional', 'regex:/@ueb\.edu\.ec$/i'],
                'cargo'                  => 'required|string|in:Director(a) financiero,Analista de presupuesto 1,Analista de presupuesto 3,Director(a) de talento humano,Rector,Administrador del sistema',
    ...
    private function enviarCredencialesEmail(User $usuario, string $contrasenaTemp): void
    {
        ...
        $urlSistema = env('FRONTEND_URL', 'http://localhost:5173');
        $extras .= $this->botonAccion($urlSistema, 'Ingresar al Sistema');
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $validated = $request->validate([
                ...
                'correo_institucional'  => ['sometimes', 'required', 'string', 'email', 'max:100', 'unique:usuarios,correo_institucional,' . $id . ',id_usuario', 'regex:/@ueb\.edu\.ec$/i'],
                'contrasena'            => 'sometimes|nullable|string|min:6',
                'cargo'                 => 'sometimes|required|string|in:Director(a) financiero,Analista de presupuesto 1,Analista de presupuesto 3,Director(a) de talento humano,Rector,Administrador del sistema',
