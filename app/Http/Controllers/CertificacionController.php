...
class CertificacionController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $query = Certificacion::with('usuario', 'unidadRequiriente');
            ...
            $data = $certificados->map(function ($cert) use ($montoTotales, $liquidadoTotales, $pendientePorCert) {
                ...
                return [
                    ...
                    'institucion'        => $cert->unidadRequiriente?->nombre_entidad ?? '-',
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Notificar a quienes pueden aprobar (según RBAC), excluyendo al propio creador
            $creador = Auth::user();
            app()->terminating(fn() => $this->notificarAprobadores($certificado, $creador, 'creacion'));
    ...
    private function notificarAprobadores($certificado, $creador, string $tipo = 'creacion'): void
    {
        try {
            // Notificar a usuarios cuyo cargo tenga permiso 'aprobar', excepto al propio creador
            ...
            $destinatarios = User::whereIn('cargo', $cargosAprobadores)
                ->where('id_usuario', '!=', $creador?->id_usuario)
                ->where(DB::raw('LOWER(estado)'), 'activo')
                ->where('recibe_notificaciones', true)
                ->get();

            $analista = $creador; // alias para los mensajes
            ...
            foreach ($destinatarios as $destinatario) {
                if ($tipo === 'creacion') {
                    ...
                    $cuerpo = "Estimado/a {$destinatario->nombres},\n\n"
                        ...
                        . "  - Fecha:       " . \Carbon\Carbon::parse($certificado->fecha_elaboracion)->format('d/m/Y') . "\n\n"
            ...
            \Log::warning('notificarAprobadores error: ' . $e->getMessage());
        }
    }
    ...
    private function notificarAprobacion($cert, $aprobador): void
    {
        ...
        if ($creador && $creador->estado === 'activo' && $creador->recibe_notificaciones) {
    ...
    {
        try {
            $entidades = UnidadRequiriente::select(
     ...
     */
    public function createUnidadRequiriente(Request $request)
    {
        try {
            $request->validate([
            ...
            $entidad = UnidadRequiriente::create([
    ...
    public function aprobar(int $id)
    {
        ...
        try {
            ...
            $aprobador = Auth::user();
            app()->terminating(fn() => $this->notificarAprobacion($cert, $aprobador));
    ...
    public function rechazar(Request $request, int $id)
    {
        ...
        try {
            ...
            // Notificar al analista que creó el certificado (diferido para no bloquear la respuesta)
            $analista = User::find($cert->id_usuario);
            if ($analista && $analista->estado === 'activo' && $analista->recibe_notificaciones && $analista->correo_institucional) {
                $revisor       = Auth::user();
                ...
                $cuerpo = "Estimado/a {$analista->nombres},\n\n"
                    ...
                    . "Sistema de Control Presupuestario\n"
                    . "Universidad Estatal de Bolívar";
                $correo = $analista->correo_institucional;
                $nombre = $analista->nombres;
                $asunto = "Certificación N.° {$cert->numero_certificado} — Rechazada";
                app()->terminating(fn() => $this->enviarCorreo($correo, $nombre, $asunto, $cuerpo));
            }
    ...
    public function reenviar(int $id)
    {
        try {
            ...
            $analista        = Auth::user();
            app()->terminating(fn() => $this->notificarAprobadores($certActualizado, $analista, 'reenvio'));
